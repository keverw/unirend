...
- **Describe the final release delta, not the branch history:** `## Unreleased` is a concise, user-facing summary of what will differ from the last release. It is not a diary of commits, review fixes, design exploration, test coverage, intermediate implementations, or the chain of bugs that led to the final fix. Write the final outcome first, then include only the migration step, behavior change, limitation, or action a user needs to know. Put design rationale, edge cases, examples, and root-cause detail in the documentation, tests, pull request description, or commit history instead.
- **Consolidate continuously by outcome:** Keep `## Unreleased` release-ready after every change. Update or replace the existing bullet for the same user-visible capability instead of appending a follow-up entry. Group related work into one bullet per final capability or fix, even when it took many commits. A branch normally needs 3 to 8 bullets. Before finishing, apply this test to every sentence: if it would be more useful in a pull request description than in published release notes, remove it. Add a separate bullet only for an independently user-visible API, behavior, dependency, platform requirement, or upgrade expectation.
- **Keep public changes represented:** When work introduces or materially changes the final user-facing release delta, update `changelog.md` under `## Unreleased` as part of the same change. If there is no `## Unreleased` section yet, create one at the bottom, below the most recent released version.
...
- **Avoid em dashes and semicolons in prose:** Don't use `—` or `;` in normal text. Write with commas, periods, or a cleaner sentence split so it reads naturally. When editing text that already has them, rewrite the sentence rather than swapping the character for a space. This is a Markdown rule, for text that gets published and read as prose. It covers the docs, the changelog, the README, and any other `.md` file. It does not cover TypeScript or JavaScript source. Comments, JSDoc, and runtime message strings there already use em dashes in most of `src/`, and rewriting them is churn rather than a fix. Within Markdown it also does not apply to code fences, inline code, tables where the punctuation is literal content, URLs, or identifiers.
