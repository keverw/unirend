...
- **Describe the final release delta, not the branch history:** `## Unreleased` is a concise, user-facing summary of what will differ from the last release. It is not a diary of commits, review fixes, design exploration, test coverage, or intermediate implementations. Write the final outcome first, then include only the migration step, behavior change, limitation, or action a user needs to know. Put design rationale, edge cases, and examples in the documentation, tests, or commit history instead.
- **Consolidate by outcome:** Group related work into one bullet per user-visible capability or fix, even when it took many commits to complete. A branch normally needs only a few bullets. Fold follow-up fixes into the bullet for the feature they refine, unless they create a separate public API, behavior, dependency, platform requirement, or upgrade expectation.
