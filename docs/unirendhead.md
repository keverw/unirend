...
- **The sub-properties of a structured object go with it.** Matching is by exact key, with one exception: `og:image:width` is its own key, so on exact matching alone it would outlive the `og:image` it belongs to and end up stating the width of the child's picture instead of the one the handler measured. A wrong claim is worse than a missing one, so a child replacing `og:image`, `og:video`, `og:audio`, `twitter:image`, or `twitter:player` takes the `:`-suffixed tags describing it along. All five are recognized on either attribute, since OpenGraph documents `property` and Twitter documents `name` but each parser takes the other, and real pages write it both ways: declare the parent on `name` and the `name` sub-properties go with it, declare it on `property` and the `property` ones do. The sweep does not cross from one attribute to the other, though. A head tag's identity is the attribute and the value together, everywhere in Unirend, which is what lets a single `<meta name="twitter:title" property="og:title">` be both of those tags at once. So a child on `name` and an envelope tag on `property` are two different tags here, exactly as they are to the [template merge](#overriding-a-template-meta) and to the [duplicate warning](#development-only-duplicate-warning), and both ship. Write the child with the attribute the envelope used, which for an `og` member is always `property`. That much holds however the envelope wrote them, as `og` members (`og: { image, 'image:width' }`) or as `tags` entries, since those are two spellings that merge into one set of tags and cannot answer to different rules.
