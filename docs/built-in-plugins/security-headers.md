...
- `hsts` (default: `false`): Controls the `Strict-Transport-Security` (HSTS) header
  ...
  - `{ maxAge: number; includeSubDomains?: boolean; preload?: boolean }`
    - `maxAge` is in whole seconds. A fraction is refused at startup rather than floored, the same as `cors.maxAge`: the header carries `max-age=<delta-seconds>`, a run of digits, so a browser cannot parse a fractional value and ignores the directive, which leaves the host unpinned by the one header whose job is pinning it.
...
- `methods`, `allowedHeaders`, and `exposedHeaders` entries are validated at startup: each must be an RFC 9110 token, since the list is written into its response header verbatim and a browser drops a value it cannot parse, and no entry may be repeated. `allowedHeaders` additionally refuses `"*"` combined with named headers. All are cases where the configured policy would otherwise differ from the policy actually served, silently.
