...
Peer ranges count too, on both sides. Bun auto-installs peer dependencies, so a peer range is a live constraint rather than a wish: a package declaring peer `react >=19` alongside an override pinning React 18 is the same backward pin as any other, and it installs without a warning. Peers bun marks optional are left out, being the ones genuinely allowed to go unsatisfied. When a package declares both a real dependency and a peer on the same name, the installed range is the one compared against, since it is the stricter and binding one. That combination is normal for a library, which develops against one version of what it broadly supports: this repo declares react as a devDependency at `^19.2.7` and a peer at `^19.0.0`.
