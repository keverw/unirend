...
What selects this handler is the mapping match, not the shape of the path, so a `folderMap` prefix that overlaps your application's routes sends the routes under it here instead of to the React 404. Every prefix is normalized to a trailing slash, so a `/about` mount covers `/about/team` but not `/about` itself. Mount folders on prefixes the app does not route. A mount at `/` covers every URL, which makes each unmatched route an asset miss and answers even your home page with this handler. This is the same discipline any static middleware needs, and the underlying mapping match also drives `request.isStaticContentMatch`, which is set whether or not `getStaticNotFoundPage` is configured.
