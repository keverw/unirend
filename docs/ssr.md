...
Ordinary unmapped routes, unmapped API misses, traversal-rejected URLs, non-GET/HEAD requests, and servers with `staticContentRouter: false` retain their normal behavior. A mapped target that exists but fails to read is a server fault rather than a miss, so it never reaches `getStaticNotFoundPage`. It follows normal SSR error handling and returns a `500` page, including `get500ErrorPage` when configured, at any file size. A target that disappears between the stat and the read is the exception, since a vanished file is a miss and takes the asset 404 like any other missing target. `getStaticNotFoundPage` is production built SSR only, since Vite owns asset handling in HMR mode, so `serveSSRWithHMR()` and `registerHMRApp()` do not offer it. `staticRequestPaths` is not limited that way and applies in both HMR and built mode.
