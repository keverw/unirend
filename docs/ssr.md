...
Ordinary routes, unmapped API misses, traversal-rejected URLs, non-GET/HEAD requests, and servers with `staticContentRouter: false` retain their normal behavior. A mapped target that fails to read is an error rather than a miss, so it never reaches `getStaticNotFoundPage`. Which error path it takes depends on size. A file at or under `smallFileMaxSize` is buffered, and its read failure is reported as a result the static hook ignores, so the request falls through to the ordinary SSR render. A larger file is streamed, and its failure surfaces when the read stream is opened, which propagates into normal SSR error handling and returns a `500` page, including `get500ErrorPage` when configured. `getStaticNotFoundPage` is production built SSR only, since Vite owns asset handling in HMR mode, so `serveSSRWithHMR()` and `registerHMRApp()` do not offer it. `staticRequestPaths` is not limited that way and applies in both HMR and built mode.
