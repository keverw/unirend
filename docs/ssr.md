...
- `staticRequestPaths?: string[]`
  - Opt-in picomatch patterns that set `request.isStaticRequest` before user plugins run. Also accepted by `StaticWebServer`. See [Advanced Asset Request Paths](#advanced-asset-request-paths).
...
- `publicFiles?: string[]`
  ...
  - OS metadata is rejected case-insensitively at config time, in any path segment rather than just the basename. Recognized names include `.DS_Store`, `._*`, `Thumbs.db`, `ehthumbs.db`, and `desktop.ini`, plus the metadata directories `.AppleDouble`, `.Spotlight-V100`, `.Trashes`, `.fseventsd`, and the like, so an entry that lives under one (e.g. `/images/.AppleDouble/metadata`) is rejected too, and the error names the offending segment (`.AppleDouble`). These are not public assets, and the static server also refuses to serve them from a `publicFolders` mount (checking every URL segment, so a junk file that slips into a declared folder can't leak). Add the recognized metadata name to `.gitignore` instead of declaring it (the `check:public-assets` script skips gitignored junk and flags only what git would commit).
...
- `publicFolders?: string[]`
  ...
  - OS metadata files (`.DS_Store`, `._*`, `Thumbs.db`, and the rest) are never served from a folder mount. They return not-found no matter what is on disk, so a junk file that lands in a declared folder can't be exposed. Gitignore them anyway, since Vite still copies them into the build, and the `check:public-assets` drift script fails CI on any that reach `public/` without being gitignored.
  ...
  - Bare `/` is rejected (mounting the client build root exposes `/index.html` and `.vite/`), as are `/assets` and anything under it (already the default mount, and a nested mount would win on longest-prefix and lose the immutable header), `.vite`, `.` and `..` segments, null bytes, backslashes, characters browsers percent-encode in URLs, and OS metadata in any segment (a recognized name like `/.Trashes`, or a metadata directory anywhere in the path like `/x/.AppleDouble`, with the error naming the offending segment). A trailing slash is tolerated and stripped, repeated slashes are collapsed before the checks (so `//` counts as the root and `/assets//` as `/assets`), and reserved names compare case-insensitively.
...
- `getStaticNotFoundPage?: (request, isDevelopment) => string | Promise<string>`
  - Standalone HTML for a static mapping that matched but has no servable file. Per-app like `get500ErrorPage`, and only used by apps that configure it. See [Advanced Asset Request Paths](#advanced-asset-request-paths).
...
The catch is what "matched no route" means on an SSR server. Every page URL here is already unmatched. That is how SSR works: a `GET '*'` catch-all hands web requests to React, and the not-found handler catches the rest. So putting a request back on the not-found path does not hide it, it puts it back on the ordinary page-rendering path. That holds wherever the call comes from. A raw plugin route and an `onRequest` hook both land in the same place, so making the call earlier is not a way around it.

What comes out of that depends on whether the active app bundle routes the URL, and the status is `404` either way. If the bundle does not route it, React falls through to its own catch-all and you get your 404 page, which is exactly what you asked for. If the bundle does route it, the app renders that page, its loaders run and return real data, and the visitor gets the real page under the `404`. The second case is the whole problem, and it is the case you are in whenever you reach for this to hide a page that exists. On a multi-bundle server it also depends on which bundle is active, since the render goes through `request.activeSSRApp`.
...
- **Call it first.** Before any expensive work. Headers and cookies set earlier in the handler are rolled back, so a `reply.setCookie()` before the trigger does not ship on the 404 and cannot give the handler away. That rollback covers the HTTP path, meaning API route handlers and page data requests over HTTP. It does not apply on the SSR internal short-circuit, whose reply belongs to the page request and is shared with every loader running in parallel, so rolling it back there could remove a header another loader had just set. Response timing is the remaining tell either way: a triggered 404 ran your handler first.

  The rollback starts when your handler is invoked, so it covers what the handler sets and not what middleware set before it. That is the right boundary rather than a gap: a hook that renews a session cookie runs on a genuine miss too, so keeping its cookie is both what a real miss looks like and what you want to happen. Only the handler's own additions are the ones that could give it away.

  The exception is a hook that sets reply state _conditionally on the matched route_. Those hooks run on both paths, but only the matched one gives them a route to branch on, so a hook that sets a header from `request.routeOptions.url` sets it on a trigger and not on a genuine miss. Taking the snapshot earlier would not help and would invert the problem, since a hook that sets a header unconditionally sets it on both paths and rolling back past the hooks would strip it from the trigger alone. If you have hooks that mutate the reply based on the matched route, keep them off the routes you gate.
...
- **A page data handler gets the page, not the transport.** When a not-found handler answers a page data request, its third argument carries `pageData` with the `pageType`, `routeParams`, `queryParams`, `requestPath`, and `originalURL` the frontend asked for, and `request.url` is that frontend URL rather than `/api/v1/page_data/<type>`. This is the same whether the request missed over HTTP or was abandoned by a trigger during an SSR render, so it is safe to put in a response, and it is what a not-found page actually wants to name. A plain API route has no `pageData` and keeps its own URL.
  ...
  `pageData` is optional, so treat it as absent rather than assuming it. It is undefined for an API route, and also for a request to the page data endpoint that did not come from a unirend loader, such as a direct `curl` with no body or a body missing `original_url`. In that case `request.url` is left as the endpoint URL, since there is no frontend view to report. Write the fallback explicitly:

  ```ts
  const page = params.pageData?.originalURL ?? request.url;
  ```
  ...
  `pageData` is specific to the not-found handler. `APIHandling.errorHandler` does not receive it, and its `request` is the ordinary Fastify one rather than a `NotFoundRequest`. Nothing is withheld there: the stripping exists so a triggered 404 cannot be told apart from a genuine miss, and a 500 has no such pair to be confused with. If an error handler wants the page a failing page data request was for, there is no params field to take it from, so read `original_url` off `request.body` yourself, which is the loader's POST body and the same field `pageData` is derived from. That applies to the HTTP path only. A page data handler that throws during an SSR render never reaches `APIHandling.errorHandler`, since the throw propagates out through the render into `get500ErrorPage`, which is the app's own 500 page and the right answer for a visitor mid-load.
...
One note specific to SSR. During the initial render, a page data handler registered on the same `SSRServer` is [short-circuited](#short-circuit-data-handlers), called in-process rather than over HTTP. A trigger there produces the not-found envelope inline, with no separate HTTP response, and the page renders its 404 state. The envelope carries the SSR request's own `request_id` and `request_timestamp`, since it was produced for that request and is piggybacking on it. The split to keep in mind: **the request is who is asking, `pageData` is what was asked for.** The request is the real one the visitor made, which is what you want for cookies, auth, and client IP, and on this path that is the browser's page request rather than a page data POST. So `request.body` and `content-type` describe the page load here, and a handler that reads them will see something different than it would on the HTTP path. `request.url`, `isPageData`, and `pageData` are the same either way, and are what a response should be built from. See [Data Loaders](./data-loaders.md#page-type-handler-fetchshort-circuit-data-loader).
...
`staticRequestPaths` is an opt-in SSR, API, plain-web, and static-web server option for classifying request URL paths before user plugins run. It uses picomatch patterns, matches only the URL path, and defaults `request.isStaticRequest` to `false`. Wildcards match path segments that begin with a dot, so `/**` covers `/.well-known/acme-challenge/token`. Matching runs on the routed path with the query and fragment removed and no other normalization, so a URL containing `..` is classified exactly as it is routed rather than as the path it would resolve to. Entries must start with `/` and cannot contain `#` or null bytes, or `?`, which is rejected at startup rather than read as picomatch's single-character wildcard, since a `?` in a path pattern is almost always a stray query string. It does not inspect static mappings or the filesystem, and it never skips plugins.
...
What selects this handler is the mapping match, not the shape of the path, so a `folderMap` prefix that overlaps your application's routes sends the routes under it here instead of to the React 404. Every prefix is normalized to a trailing slash, so a `/about` mount covers `/about/team` but not `/about` itself. Mount folders on prefixes the app does not route. Built SSR rejects a root `/` folder mount at startup because it would inspect the filesystem for every page request and could expose build metadata. The underlying mapping match also drives `request.isStaticContentMatch`, which is set whether or not `getStaticNotFoundPage` is configured.

Ordinary unmapped routes, unmapped API misses, traversal-rejected URLs, non-GET/HEAD requests, and servers with `staticContentRouter: false` retain their normal behavior. A mapped target that exists but fails to read is a server fault rather than a miss, so it never reaches `getStaticNotFoundPage`. It follows normal SSR error handling and returns a `500` page, including `get500ErrorPage` when configured, at any file size. A target that disappears between the stat and the read is one exception, since a vanished file is a miss and takes the asset 404 like any other missing target. A path under a directory the server cannot search is the other, since nothing under it resolves at all. `getStaticNotFoundPage` is production built SSR only, since Vite owns asset handling in HMR mode, so `serveSSRWithHMR()` and `registerHMRApp()` do not offer it. `staticRequestPaths` is not limited that way and applies in both HMR and built mode.
...
Files in a Vite app's `public/` folder (favicon files, `robots.txt`, web manifests, logos) are copied verbatim to the client build root and referenced by literal URL. In dev/HMR mode Vite's middleware serves them implicitly, but the production static router is driven by a declared list and only touches disk for known files. Every intended public asset must therefore be declared, individual files with `publicFiles` and whole subfolders with `publicFolders` (or it 404s in production). OS metadata files (`.DS_Store`, `Thumbs.db`, and friends) are never intended assets, so `.gitignore` them to keep them out of the build:
...
- **OS-junk guard**: The static server never serves OS metadata such as `.DS_Store`, `._*`, `Thumbs.db`, `ehthumbs.db`, and `desktop.ini` from a `publicFolders` mount, so one that slips into a declared folder can't leak at runtime even if it reaches the build. Declaring one is still a mistake: `publicFiles` rejects these names at config time. The repo-level public-assets check also flags them anywhere under `public/` (including inside a declared `publicFolders` subtree), but it's gitignore-aware, since Vite copies them into the build and a plain static host or CDN can then expose them: a junk file `git check-ignore` reports as ignored won't be committed and is skipped, while one git would commit (unignored, or already tracked) is a hard error. Add their names to `.gitignore` (the check then passes without deleting the copy the OS keeps recreating), never declare them.

Projects generated from the starter templates declare these lists as `PUBLIC_FILES` and `PUBLIC_FOLDERS` in each app's `consts.ts`, and the repo-level `bun run check:public-assets` script (part of `bun run check`) fails CI when the lists drift from the actual `public/` folder in either direction (files under a declared folder are covered automatically) or when un-gitignored OS junk appears under `public/` (gitignored junk is skipped, since git won't commit it). The script locates each app's lists through the `public-assets.config.json` scaffolded next to `consts.ts`, and [multi-app projects](#multi-app-ssr-support) declare one entry per app there. See [Starter Templates](./starter-templates.md).
...
One caveat when uploading the build output: a CDN or object store serves whatever it is handed, so exclude OS metadata files (`.DS_Store`, `._*`, `Thumbs.db`, and the rest) from the upload. The built server 404s them from a folder mount, but a plain static host does not, so a stray one that rode along in `public/` is publicly reachable unless your upload step filters it out. Adding them to `.gitignore` keeps them out of the repo, so a build from a clean checkout like CI won't contain them, but the OS can drop a fresh `.DS_Store` into your working tree at any time, and a build run from there copies it in. Filtering at the upload step is the reliable guard: the `check:public-assets` gate fails CI on junk that was committed, but by design it skips a gitignored copy the OS just recreated locally, and that copy still rides a local build into the upload.
...
`request.activeSSRApp` is a plain `string`, so a comparison against a bundle key is not checked for you. A typo compiles, runs, and never matches, which stops the gate working without saying so. Which way it breaks depends on how you wrote it. In the form below the check is true on every bundle, so the route 404s everywhere including the one it was meant to serve. Written the other way round, as `=== 'app-shelf'`, the same typo never fires and the handler answers on every bundle:
...
`'__default__'` is refused here, unlike in `is()` and the case objects. The app the server was created with is registered for you, so there is no register call to check, and passing that key to one throws.
...
- `DEFAULT_APP_BUNDLE_KEY` is exported from `unirend/server` for code that compares against the server's own bundle without going through `defineAppBundles()`. It is the same `'__default__'` value, but a typo in the imported name is a compile error where a typo in the string literal is a gate that silently never matches.
...
- An empty declaration, `defineAppBundles()`, is legal, so you can adopt the helper before there is a second bundle to name. The app the server was created with is passed to `serveSSRWithHMR()` or `serveSSRBuilt()`, not to `registerBuiltApp()` / `registerHMRApp()`, and registration is what assigns a key, so that app never had one of your choosing. It is selectable as `'__default__'`. Serving only that app, before any others are registered, is a normal state and is where every app starts. A declared key only becomes real when that same key is passed to `registerBuiltApp()` / `registerHMRApp()`, and the app given to `serveSSRWithHMR()` / `serveSSRBuilt()` goes through neither. So inventing a name for it, as `defineAppBundles('marketing')` without a matching `registerBuiltApp('marketing', …)`, type-checks and gates nothing: nothing ever selects `'marketing'`, so `bundles.is(request, 'marketing')` is false on every request while that app keeps arriving as `'__default__'`. Declare nothing instead and gate on `'__default__'`, then add each key when its bundle is actually registered, which leaves the gates already written unchanged. With no keys, `'__default__'` is the only key that type-checks, and `is()` and the three dispatchers all select it. `key()` cannot be called at all rather than throwing, since there is no key it could hand back.
- Because of that, `defineAppBundles(...names)` spread from a `string[]` that is empty at runtime is now accepted rather than refused. Nothing breaks silently when it happens, since every method throws on any key other than `'__default__'`, so the empty list surfaces at the first real use with a message naming the key. That is later than it used to be, though: the old zero-key throw fired at module load, so an accidentally empty list failed the boot, where a gate reached only on a request now reports on that request instead. `dispatch()` still checks its case keys when the handler is built, and `key()` at the registration call, so how early you find out depends on which of them the app uses.
