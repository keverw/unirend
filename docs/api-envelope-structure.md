...
- Pass the `getRequestID` server option (on SSR, API, Static, and Redirect servers) to control generation, for example to adopt an upstream/proxy `X-Request-ID` from a trusted header. Returning `undefined` or an empty string opts out, in which case the helpers fall back to `"unknown"`.
...
**Note:** If your API uses HTTP redirects, they should only be used in pure API endpoints (raw Fastify routes registered directly via custom plugins, not routes serving page data loaders) and should not be added by middleware before page data handler routes. Envelope-first shortcut API routes (registered via `server.api.*` and passed a `ControlledReply` wrapper used only for setting headers, or cookies when the cookies plugin is registered, rather than sending responses directly) do not support HTTP-level redirects. For standard REST/JSON API operations (such as submitting a form, resource creation, or actions that trigger redirects), the recommended practice is to return a standard API success response (e.g., `201 Created` or `200 OK`) and include the target destination URI or target URL inside the JSON payload data (for example: `data: { url: "/v1/items/123" }`). The frontend client then handles the UI navigation transition itself. This keeps the API decoupled from routing concerns.
