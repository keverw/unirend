...
- Create responses:
  ...
  - `await APIResponseHelpers.sendErrorEnvelope(request, reply, statusCode, errorEnvelope)` - Sends an error envelope immediately using the raw/hijacked response path. This applies shared security headers first when the built-in `securityHeaders` plugin is present, then writes JSON directly to the socket. Because it uses the raw hijack path, normal Fastify `onSend` hooks do not run for that response.
