...
- `singleAssets` URL keys get a leading slash ensured now, matching `StaticWebServer` from the `unirend` npm package. Previously a key written as `robots.txt` was used verbatim, so it never matched any request (requests always start with `/`) and the asset silently 404ed.
