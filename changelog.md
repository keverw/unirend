...
- Fixed `processTemplate` stripping nearly every named `<meta>` tag from the `index.html` template, including `<meta name="viewport">`. Previously the only named meta that survived was `apple-mobile-web-app-title`, which was special-cased by name, so every app served pages with no viewport meta and mobile browsers laid them out at desktop width and scaled the result down, defeating responsive CSS regardless of your media queries. The starter templates ship a viewport meta in their `index.html` and were affected.

  Head tags are now split by ownership. `UnirendHead` owns the per-page SEO tags, and the template's copies of those are always stripped: `<title>`, `<meta name="description">`, the OpenGraph tags (`og:*`), and the Twitter card tags (`twitter:*`). Everything else in the template is a baseline that is served as-is, including `<meta name="viewport">`, `<meta charset>`, `<meta name="theme-color">`, `<meta name="robots">`, `<meta property="og:site_name">`, and any custom tags you add. `og:site_name` is exempt from the `og:` rule because it names the site rather than the page.

  A page can still override a baseline tag by declaring a `<meta>` with the same `name`, `property`, or `http-equiv` through `UnirendHead`: the page's version wins and the template's copy is dropped, so the served head never carries both, and navigating to a page that doesn't override it brings the template's version back. This matches how `<html>` and `<body>` attributes already behaved, where the template's `<html lang="en">` is a baseline a page can override. The page-owned SEO metas (`description`, `og:*`, `twitter:*`) are stripped unconditionally rather than kept as a baseline because they describe the individual page: a template default would put a generic, stale description or `og:title` on every page that forgot to set its own. Set them in the page itself, and in your error components too, since a standalone error page renders outside the normal layout. Note that `<meta>` tags accumulate across `UnirendHead` instances rather than overriding by name, so a layout that sets `description` and a page that also sets it produce two `description` metas: a layout is the right place for a meta only when every page under it should carry that exact tag. `<title>` is stripped for a further reason, that it is not part of the reconciled meta baseline: React will not remove a `<title>` already sitting in the head, so keeping the template's would leave the document with two once a page renders its own, and a document takes its title from the first one in tree order.

  The override holds across client-side navigation, not only on the server-rendered page. `UnirendHead` now reconciles the template's metas on the client the way it already did for `<html>`/`<body>` attributes: it removes a template meta from the head while a page overrides it and restores it once nothing does. Without this a template meta stripped for the landing page would never come back after navigating to a page that doesn't override it, and one left in the head would end up sitting alongside the meta React hoists on the next navigation. Two new implementation details are visible in the served HTML: `window.__UNIREND_TEMPLATE_METAS__` carries the template's meta baseline to the client (the companion to the existing `window.__UNIREND_TEMPLATE_ATTRS__`), and template-owned metas in the head are tagged with a `data-unirend-template-meta` attribute so the client can tell them apart from the ones React hoists.

  Applies to SSR (dev and built) and SSG alike, since all three paths run through the same template processing. If you worked around the missing viewport by redeclaring it through `UnirendHead`, that workaround is no longer needed and can be removed. If you had page-owned SEO tags sitting unused in your `index.html`, note that OpenGraph and Twitter tags there are now stripped rather than silently kept, so move them into `UnirendHead`.
