...
interface ServeSSROptions<
  H extends APIResponseHelpersClass = APIResponseHelpersClass,
> {
  /** Server-wide pathname glob patterns that decorate requests before user plugins run. */
...
interface ServeSSRBuiltOptionsBase<
  H extends APIResponseHelpersClass = APIResponseHelpersClass,
> extends ServeSSROptions<H> {
  /** Standalone HTML for a mapped static asset whose target is missing. */
...
declare module 'fastify' {
  interface FastifyRequest {
    ...
    isStaticAsset: boolean;
    /**
     * Set to `true` once a configured static-content mapping matches this
     * request, whether it later serves the target, finds it missing, or rejects
     * OS metadata from a folder mapping. It is initialized to `false` for
     * every request handled by SSR, API, and plain servers.
     *
     * Unlike `isStaticAsset`, this remains `true` when the mapped target is
     * missing, cannot be served, or is rejected as OS metadata, so an API or
     * plain server `notFoundHandler` can choose an asset-specific response. It
     * is set by static routing after user `onRequest` hooks, not by
     * `staticRequestPaths` classification.
     */
    isStaticContentMatch: boolean;
    /** Early opt-in pathname classification, independent of static serving. */
