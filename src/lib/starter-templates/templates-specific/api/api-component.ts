    ...
    this.startPromise = (async () => {
      try {
        ...
        this.server = serveAPI({
          ...
          // clientInfo adds a correlation ID (for tracing across the SSR → API hop),
          // forwarded client IP/User-Agent, and X-Request-ID/X-Correlation-ID response
          // headers. The request ID itself (the envelope API request_id) is generated by the
          // server automatically — see the getRequestID server option to customize it.
