...
// NOTE: `requestID` is declared by the core framework (see types.ts) since it is
// generated by the server itself, not this plugin.
...
export interface ClientInfoConfig {
   ...
   * `getRequestID` server option), not this plugin.
   */
...
export function clientInfo(config: ClientInfoConfig = {}): ServerPlugin {
  return (pluginHost: PluginHostInstance, _options: PluginOptions) => {
    ...
    // Note: request.requestID is decorated by the server, not here.
    ...
    pluginHost.addHook('onRequest', async (request, reply) => {
      ...
      // Read the request ID generated by the server (defaults to a ULID, or the
      // getRequestID server option). Empty string only if a custom getRequestID
      // opted out by returning undefined/empty.
      ...
      // request.requestID is already set by the server (used by envelope helpers
      // and SSR forwarding); this plugin only reflects it into clientInfo.
      ...
      // Guard against empty values: when getRequestID opts out, requestID (and
      // its derived correlationID) can be empty — don't emit blank headers.
